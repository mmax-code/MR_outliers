...
setwd(".../Github/Multivariable/Simulation/")
...
simulation = function(seed,num.ind = 200000,var.expl1 = c(0.1,0.1,0.1,0.1),
                      ...
                      ss.effects = c(1,1,1,1,1), alpha = 0.05, outlier.func){
  ...
  var.eps5 = ((1 - var.expl1[4]) / var.expl1[4]) * (beta4^2 %*% var.u)
  ...
  outlier = X %*% rho + epsilon[ ,4]  # correlaton zw. risk und outlier (sigma 4x4), aber nicht mit dem confounder
  ...
  rf4 = U %*% beta4 + rnorm(num.ind, 0,sqrt(var.eps5))
  
  
  RF = as.matrix(cbind(rf1,rf2,rf3,rf4,outlier))
  
  # Second-Stage regression
  # fix variance explained in second-stage regression
  var.rf = cbind(var(rf1),var(rf2),var(rf3),var(rf4),var(outlier))
  var.eps = ((1 - var.expl2) / var.expl2) * ((ss.effects^2 %*% t(var.rf)) +
                                               ...
                                               2*ss.effects[1]*ss.effects[5]*cov(outlier,rf1) +
                                               2*ss.effects[2]*ss.effects[5]*cov(outlier,rf2) +
                                               2*ss.effects[5]*ss.effects[3]*cov(outlier,rf3) +
                                               2*ss.effects[5]*ss.effects[4]*cov(outlier,rf4) +
                                               2*ss.effects[4]*ss.effects[1]*cov(rf1,rf4) +
                                               2*ss.effects[4]*ss.effects[2]*cov(rf4,rf2) +
                                               2*ss.effects[4]*ss.effects[3]*cov(rf4,rf3)) 
  ...
  result = list(coeffs,lambda, ss.effects, mvmr.lm,q.values,q.outliers,q.lm,q.sanderson, sanderson.outliers, sanderson.lm,q.adjust,
                median.outliers,median.lm, mr.presso, outlier.indices,lm1,lm2,lm3,lm4)
  names(result) = c("coeffs","lambda","causal.effects","model.outlier",
                    ...
                    "mr.presso", "outlier.indices","lm.full","lm.q","lm.qsand","lm.qmedian")
...
x1 = foreach(n = 1:1000) %dopar% simulation(seed = n, num.ind = 500000,var.expl1 = c(0.15,0.15,0.15,0.15), var.expl2 = 0.5,
                                            sigma = sigma,outlier.perc = 0.15, ss.effects = c(0,1,-1,0.5,1),alpha = 0.05,
                                            runif(floor(0.15*100),5,6))
print(difftime(Sys.time(), t, units = 'sec'))

save(x1, file = "Multivariable_Outliers.RData")
...
load("Multivariable_Outliers.RData")
...
lm.presso = list()
for (i in 1:1000){
  full[[i]] = c((x1[[i]]$lm.full$coefficients[1]-0), (x1[[i]]$lm.full$coefficients[2]-1), (x1[[i]]$lm.full$coefficients[3]-(-1))) 
  lm.q[[i]] = c((x1[[i]]$lm.q$coefficients[1]-0) , (x1[[i]]$lm.q$coefficients[2]-1) ,(x1[[i]]$lm.q$coefficients[3]-(-1)))
  lm.sanderson[[i]] = c((x1[[i]]$lm.qsand$coefficients[1]-0),(x1[[i]]$lm.qsand$coefficients[2]-1),(x1[[i]]$lm.qsand$coefficients[3]-(-1)))
  lm.median[[i]] = c((x1[[i]]$lm.qmedian$coefficients[1]-0),(x1[[i]]$lm.qmedian$coefficients[2]-1),(x1[[i]]$lm.qmedian$coefficients[3]-(-1)))
  lm.presso[[i]] = c((x1[[i]]$mr.presso$`Main MR results`$`Causal Estimate`[4]-0),(x1[[i]]$mr.presso$`Main MR results`$`Causal Estimate`[5]-1),(x1[[i]]$mr.presso$`Main MR results`$`Causal Estimate`[6]-(-1)))
}
...
names = c(rep("Full model",3000),rep("Standard",3000),rep("Sanderson",3000),rep("Median-adjusted",3000),rep("MR-Presso",3000))
theta = rep(c("theta_1", "theta_2", "theta_3"),5000)
df = data.frame(coeff = coefficients, method = names,theta = theta)
df$method = as.factor(df$method)
df$method = ordered(df$method, levels = c("Full model", "Standard", "Sanderson",
                                          "MR-Presso", "Median-adjusted"))

p = ggplot(df, aes(x=method, y=coefficients,fill=theta)) + 
...
ind = which(x1[[i]]$outlier.indices == TRUE)
data_w = data.frame(value = x1[[i]]$q.values[ind],outlier = rep("outlier",length(ind))) 
data_wo = data.frame(value = x1[[i]]$q.values[-ind], outlier = rep("no-outlier", length(x1[[i]]$q.values[-ind])))
data = rbind(data_w, data_wo)


# basic histogram
p = ggplot(data, aes(x=value)) + 
  geom_histogram(data=subset(data,outlier == 'outlier'),binwidth=4, fill = "#F8766D", alpha = 0.6) +
  geom_histogram(data=subset(data,outlier == 'no-outlier'),binwidth=4, fill = "#619CFF", alpha = 0.6)+
  geom_vline(xintercept=3.841, linetype="dashed", color = "#00BA38", size=0.7) +
  labs(title="",x="q-Statistics")
p + theme_minimal()
...
lm.presso = list()
for (i in 1:1000){
  full[[i]] = c((x1[[i]]$lm.full$coefficients[1]-0)^2, (x1[[i]]$lm.full$coefficients[2]-1)^2, (x1[[i]]$lm.full$coefficients[3]-(-1))^2) 
  lm.q[[i]] = c((x1[[i]]$lm.q$coefficients[1]-0)^2 , (x1[[i]]$lm.q$coefficients[2]-1)^2 ,(x1[[i]]$lm.q$coefficients[3]-(-1))^2)
  lm.sanderson[[i]] = c((x1[[i]]$lm.qsand$coefficients[1]-0)^2 ,(x1[[i]]$lm.qsand$coefficients[2]-1)^2  ,(x1[[i]]$lm.qsand$coefficients[3]-(-1))^2  )
  lm.median[[i]] = c((x1[[i]]$lm.qmedian$coefficients[1]-0)^2 ,(x1[[i]]$lm.qmedian$coefficients[2]-1)^2 , (x1[[i]]$lm.qmedian$coefficients[3]-(-1))^2)
  lm.presso[[i]] = c((x1[[i]]$mr.presso$`Main MR results`$`Causal Estimate`[4]-0)^2 , (x1[[i]]$mr.presso$`Main MR results`$`Causal Estimate`[5]-1)^2,(x1[[i]]$mr.presso$`Main MR results`$`Causal Estimate`[6]-(-1))^2)
}
...
### Density Q-Plot without outliers


cl = makeCluster(20)
registerDoParallel(cl)
x1 = foreach(n = 1:1000) %dopar% simulation(seed = n, num.ind = 500000,var.expl1 = c(0.15,0.15,0.15,0.15), var.expl2 = 0.5,
                                            sigma = sigma,outlier.perc = 0.0, ss.effects = c(0,1,-1,0.5,0),alpha = 0.05,
                                            runif(floor(0*100),5,6))

save(x1, file = "Multivariable_wo_Outliers.RData")

load("Multivariable_wo_Outliers.RData")

object = x1

Qstat = list()
for (i in 1:1000){
  tmp1 = sum(object[[i]]$q.values)
  tmp2 = sum(object[[i]]$q.sanderson)
  tmp3 = sum(object[[i]]$q.lambda)
  Qstat[[i]] = list(tmp1,tmp2,tmp3)
}


tmp1 = as.numeric(lapply(Qstat, function(l) l[[1]]))
tmp2 = as.numeric(lapply(Qstat, function(l) l[[2]]))
tmp3 = as.numeric(lapply(Qstat, function(l) l[[3]]))

tmp1 = tmp1[complete.cases(tmp1)]
tmp2 = tmp2[complete.cases(tmp2)]
tmp3 = tmp3[complete.cases(tmp3)]

data1 = data.frame(qvalues = tmp1,name = rep("Standard",1000))
data2 = data.frame(qvalues = tmp2,name = rep("Sanderson",1000))
data3 = data.frame(qvalues = tmp3,name = rep("Median-adjusted",1000)) 
...
p  = ggplot(data, aes(x=qvalues, color=Method, fill=Method)) +
  geom_density(alpha=0.3,size=1) +
  geom_vline(xintercept=120.990, linetype="dashed", color = "black", size=0.7)+
  stat_function(fun = dchisq, args = list(df = 97), color = "black", linetype="dashed") +
