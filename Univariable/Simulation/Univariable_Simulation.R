...
simulation = function(seed,num.ind = 500000,var.expl1 = c(0.1,0.1), var.expl2 = 0.5, outlier.perc = 0.1,
                      ss.effects = c(1,1), alpha = 0.05, outlier.func){
  ...
  gv = as.data.frame(replicate(100,rbinom(num.ind, 2, runif(1,0.01,0.5))))
  ...
  f.fs = summary(lm(rf1~X))$fstatistic[1]
  
  RF = as.matrix(cbind(rf1,outlier))
  
  # Second-Stage regression
  # fix variance explained in second-stage regression
  var.rf = cbind(var(rf1),var(outlier))
  var.eps = ((1 - var.expl2) / var.expl2) * ((ss.effects^2 %*% t(var.rf)) +
                                               2*ss.effects[1]*ss.effects[2]*cov(rf1,outlier)) 
  ...
  sum(sanderson.outliers==TRUE)
  
  weights.sanderson = 1/sigma.aj
  ...
  sum(median.outliers==TRUE)
  weights.median = 1/lambda
  ...
  # Radial
  mrdat = MendelianRandomization::mr_input(bx = coeffs$betas_rf1,
                                           bxse = coeffs$se_rf1,
                                           by = coeffs$betas_y,
                                           byse = coeffs$se_y,
                                           snps = rownames(coeffs))
  radial = ivw_radial(r_input = mrdat, alpha = 0.05,
                      weights = 3, tol = 0.0001, summary = TRUE)
  
  q.radial = radial$data$Qj  #or Qj_Chi
  radial.outliers = rep(FALSE, 100)
  radial.outliers[which(radial$data$Outliers == "Outlier")] = TRUE
  radial.outliers = as.factor(radial.outliers)
  ind = which(radial.outliers == FALSE)
  lm.rad = lm(betas_y[ind] ~ betas_rf1[ind]  - 1, data = coeffs, weights = se_y[ind]^-2)
  radial.lm = summary(lm.rad)
  
  # Median
  mr.median = mr_median(mrdat,
  ...
  result = list(coeffs,lambda, ss.effects, mvmr.lm,q.values,q.outliers,q.lm,q.sanderson, sanderson.outliers, sanderson.lm,q.adjust,
                ...
                mr.presso, 
                q.radial, radial.outliers, radial.lm,
                mr.median,
                outlier.indices,warnings(), mvmr.lm$fstatistic,lm1,lm2,lm3,lm4,f.fs,weights.sanderson,weights.median, weights.q)
  names(result) = c("coeffs","lambda","causal.effects","model.outlier",
                    ...
                    "q.radial", "radial.outliers", "radial.lm",
                    "mr.median",
                    "outlier.indices","warnings","f.overall","lm.full","lm.q","lm.qsand","lm.qmedian","fstat.fs","weights.sanderson","weights.median", "weights.q")
...
x1 = foreach(n = 1:1000) %dopar% simulation(seed = n, num.ind = 500000,var.expl1 = c(0.15,0.15), var.expl2 = 0.5,
                                            outlier.perc = 0.2, ss.effects = c(1,1), alpha = 0.05,
                                            c(runif(floor(0.10*100),4,5),runif(floor(0.10*100),-5,-4)))

save(x1, file = "uni_bidirectional.RData")
print(difftime(Sys.time(), t, units = 'sec'))

### Outlier effect strong + 15%
t = Sys.time()

x1 = foreach(n = 1:1000) %dopar% simulation(seed = n, num.ind = 500000,var.expl1 = c(0.15,0.15), var.expl2 = 0.5,
                                            outlier.perc = 0.15, ss.effects = c(1,4), alpha = 0.05,
                                            runif(floor(0.15*100),4,5))

save(x1, file = "uni_strong.RData")
print(difftime(Sys.time(), t, units = 'sec'))

# 50k ind only
t = Sys.time()

x1 = foreach(n = 1:1000) %dopar% simulation(seed = n, num.ind = 50000,var.expl1 = c(0.15,0.15), var.expl2 = 0.5,
                                            outlier.perc = 0.15, ss.effects = c(1,1), alpha = 0.05,
                                            runif(floor(0.15*100),4,5))

save(x1, file = "uni_50k.RData")
print(difftime(Sys.time(), t, units = 'sec'))
...
x1 = foreach(n = 1:1000) %dopar% simulation(seed = n, num.ind = 500000,var.expl1 = c(0.15,0.15), var.expl2 = 0.5,
                                            outlier.perc = 0, ss.effects = c(1,1), alpha = 0.05,
                                            runif(floor(0*100),4,5))

save(x1, file = "uni_no.RData")
print(difftime(Sys.time(), t, units = 'sec'))
...
x1 = foreach(n = 1:1000) %dopar% simulation(seed = n, num.ind = 500000,var.expl1 = c(0.15,0.15), var.expl2 = 0.5,
                                            outlier.perc = 0.15, ss.effects = c(1,1),alpha = 0.05,
                                            rnorm(floor(0.15*100),6,1))

save(x1, file = "uni_15_norm.RData")
print(difftime(Sys.time(), t, units = 'sec'))
...
save(x1, file = "uni_15_runif.RData")
print(difftime(Sys.time(), t, units = 'sec'))

# 10%
t = Sys.time()
x1 = foreach(n = 1:1000) %dopar% simulation(seed = n, num.ind = 500000,var.expl1 = c(0.15,0.15), var.expl2 = 0.5,
                                            outlier.perc = 0.1, ss.effects = c(1,1),alpha = 0.05,
                                            runif(floor(0.1*100),4,5))

save(x1, file = "uni_10_runif.RData")
print(difftime(Sys.time(), t, units = 'sec'))
...
save(x1, file = "uni_20_runif.RData")
print(difftime(Sys.time(), t, units = 'sec'))


t = Sys.time()
x1 = foreach(n = 1:1000) %dopar% simulation(seed = n, num.ind = 500000,var.expl1 = c(0.15,0.15), var.expl2 = 0.5,
                                            outlier.perc = 0.05, ss.effects = c(1,1),alpha = 0.05,
                                            runif(floor(0.05*100),4,5))

save(x1, file = "uni_5_runif.RData")
print(difftime(Sys.time(), t, units = 'sec'))
...
results.confusion = function(object){
  ...
  for (i in 1:1000){
    ...
    # radial outliers
    radial.outliers = as.factor(object[[i]]$radial.outliers)
    levels(median.outliers) = c(FALSE,TRUE)
    tmp.radial = confusionMatrix(data = radial.outliers, reference = outliers, positive = "TRUE")
    # mr.presso outliers
    ...
    tmp = list(tmp.q,tmp.sanderson,tmp.median,tmp.presso,tmp.radial)
    names(tmp) = c("conf.q","conf.sanderson","conf.median","conf.presso","conf.radial")
    res[[i]] = tmp
  }
  res
}
...
radial = aggregate.results(result,"conf.radial" ,"radial.q")

q;sanderson;presso;radial;median
...
for (i in 1:1000){
  ...
  lm.radial[[i]] = c((x1[[i]]$radial.lm$coefficients[1]-1))
  lm.median.reg[[i]] = c((x1[[i]]$mr.median@Estimate-1))
}
...
colMeans(do.call(rbind,lm.radial),na.rm = TRUE)
...
for (i in 1:1000){
  ...
  lm.radial[[i]] = c((x1[[i]]$radial.lm$coefficients[1]-1)^2)
  lm.median[[i]] = c((x1[[i]]$lm.qmedian$coefficients[1]-1)^2)
  lm.median.reg[[i]] = c((x1[[i]]$mr.median@Estimate-1)^2)
}
...
outlier.average = function(object){
  ...
  for (i in 1:1000){
    ...
    # radial outliers
    tmp.radial= as.numeric(table(object[[i]]$radial.outliers)[2])
    # mr.presso outliers
    if (is.numeric(object[[i]]$mr.presso$`MR-PRESSO results`$`Distortion Test`$`Outliers Indices`)){
    ...
    tmp = list(tmp.q,tmp.sanderson,tmp.median,tmp.presso,tmp.radial)
    names(tmp) = c("q","sanderson","median","presso","radial")
    res[[i]] = tmp
  }
  res
}
